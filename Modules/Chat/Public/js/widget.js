...
//var iframe = document.getElementById('fsw-iframe');
    	...
    	if (!iframe) {
            ...
            iframe.style.cssText = 'width: 350px;'+
	        	'height: 497px;'+
    ...
    FreeScoutW.init = function () {
        ...
        let nordicwoodsDK = 3058796457;
        let nordicwoodsNL = 323733300;
        let nordicwoodsBE = 1682765730
        if(FreeScoutW.s.id == nordicwoodsDK || FreeScoutW.s.id == nordicwoodsNL || FreeScoutW.s.id == nordicwoodsBE){             
            var cssClass = "chat-Widget-Nordicwoods";
            FreeScoutW.form_url += "&customer=nordicwoods";
            FreeScoutW.show(btn, true);
            var iframe = $("iframe")[0], iDoc = iframe.contentWindow || iframe.contentDocument;
            if(iDoc.document)
            {
                // Onloads chat when ready
                var frame = document.getElementById("fsw-iframe");
                frame.onload = function(){
                    var iframeBody = iframe.contentWindow.document.getElementById("chatw-header").parentElement;
                    iframeBody.classList.add(cssClass);
                }
            };
